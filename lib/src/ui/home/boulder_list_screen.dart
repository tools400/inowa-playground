...
class _BoulderListScreenState extends State<BoulderListScreen> {
  ...
  Widget bluetoothStatus(BleStatus? status) {
    status ??= BleStatus.unknown;

    ConsoleLog.log('BLE status: $status');
    ...
    switch (status) {
      ...
      default:
        text = "Waiting for Bluetooth status $status ...";
        color = Colors.amber;
    }
  ...
  Widget build(BuildContext context) =>
      Consumer<BleStatus?>(builder: (_, BleStatus? bleStatus, __) {
        Widget panel;
        switch (_pageMode) {
          case PageMode.sort:
        ...
        return Scaffold(
            appBar: AppBar(
              title: Column(
                children: [
                  Text(AppLocalizations.of(context)!.mnu_Problems),
                  bluetoothStatus(bleStatus),
                ],
              ),
